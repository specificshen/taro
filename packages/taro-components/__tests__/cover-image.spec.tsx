...
import { CoverImage } from '../src/components/cover-image/cover-image'
...
describe('CoverImage', () => {
  ...
  it('should show an cover-image', async () => {
    page = await newSpecPage({
      ...
      template: () => <taro-cover-image-core src={IMAGE} />,
    })
  ...
  it('events', async () => {
    let loadDetail
    const onLoad = jest.fn((e) => {
      loadDetail = e.detail
    })
    const onError = jest.fn()
    page = await newSpecPage({
      components: [CoverImage],
      template: () => <taro-cover-image-core src="" onLoad={onLoad} onError={onError} />,
    })
    ...
    const detail = { width: img?.width, height: img?.height }
    img?.dispatchEvent(new CustomEvent('load', { detail }))
    expect(onLoad).toHaveBeenCalledTimes(1)
    expect(loadDetail).toEqual(detail)
    // expect(onError).toHaveBeenCalledTimes(1)
